...
- 소스코드에 필요한 글자 수를 무조건적으로 짧게 하는 것이 아니다. 읽기 좋고 유지보수하기 쉬운 코드를 지향한다는 것이다.
...
- 프로그램 설계가 오류를 원천적으로 방지할 수 있는 구조이다. 즉 컴파일러가 프로그램의 작동하는 방식에 대해 더 많이 알고 있다. 이 말은 곧 프로그램에 더 많은 정보를 덧붙여야 한다는 뜻이기도 하다. (trade off)

- JVM이 제공하는 안전성을 누린다.

- 읽기 전용 변수(val)와 불변 클래스(data)로 멀티 스레드 애플리케이션의 안전성을 보장한다.

- 백 번 말하고 있지만 컴파일 시점에 오류를 발견, 특히 NPE를 발견할 수 있다!
...
- 클래스 타입 변환 예외를 방지한다. 어떤 객체를 다른 타입으로 변환하기 전 타입을 미리 검사하지 않으면 예외가 발생한다. **코틀린은 타입 검사와 캐스트를 한 연산자로 수행**한다.
...
- 더 나은 자바
    ...
    - 서버 프로그래밍
        - 자바와 호환가능(클래스 상속, 어노테이션 활용 등)하면서도 `NullPointerException`을 사전에 방지할 수 있다.
        - Spring, Ktor, http4k

- 다중 플랫폼

    - JVM 외에서도 돌아간다. 자바스크립트/이진 코드로 컴파일이 가능하며, 웹어셈블리 바이너리 형식으로도 컴파일할 수도 있다!
    - expect/actual
    - 종류
        - 데스크톱
        - iOS
    ...
    > 코틀린의 크로스 플랫폼: 비즈니스 로직은 하나의 언어로 통일하기 위해. 로직을 짠 뒤 UI는 스위프트, 코틀린으로 보내는 느낌으로 돌아간다고~
