...
```
- 단순히 프로퍼티/메서드 하나만 꺼내서 쓰겠다는 의도 드러남
- 간결함
...
- 함수형 인터페이스를 명시적으로 구현하는 익명 객체를 만들어 넘길 수도 있으나 이 경우 매번 새로운 각체가 생성된다. 람다를 사용하면 람다에 해당하는 익명 객체가 재사용된다. (외부 변수를 캡처하지 않는 경우에만! 왜냐면 외부 변수 캡처하면 그때그때 다른 인스턴스 써야 하니까)

## SAM 변환

> Single Abstract Method (함수형 인터페이스의 근본 개념)
