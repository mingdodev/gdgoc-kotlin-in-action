...
> 모든 파라미터에 기본값을 정의하면 컴파일러가 자동으로 파라미터 없는 생성자를 만들어줌
...
```
- 자바: 인스턴스 생성을 막기 위해 private 생성자 사용

```kotlin
// Kotlin: math_utils.kt
// 별도의 클래스 없이 파일 최상위에 함수를 선언
...
```kotlin
fun main() {
    val processed = hashSetOf(Customer("장민서", 1234))
    println(processed.contains(Customer("장민서", 1234)))
    // false
}
```
...
- 이 메서드들을 자동으로 생성한다. IDE의 힘을 빌리지 않더라도!
...
- 주 생성자에 정의된 프로퍼티를 고려해 `equals, hashCode`를 만든다.
    - 모든 프로퍼티를 고려한다. 따라서 비즈니스 의미에 따라 변경해야 할 경우는 직접 오버라이드 필요.
- 프로퍼티가 var이어도 되지만 val로 불변을 유지하는 걸 추천. 특히 컨테이너에 담기는 경우에는 프로퍼티 변경으로 해시값이 바뀌면 객체의 동등성 비교가 어려워진다!
...
### 동반 객체(Companion Object): 팩토리 메서드와 정적 멤버가 들어갈 장소
